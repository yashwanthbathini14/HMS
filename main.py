...
def home():
    ...
    symptom_list = list(sym_des.columns)  # or sym_des['Symptom'].tolist() if symptoms are rows

    if request.method == 'POST':
        # Get user input (text field or multiple checkboxes)
        symptoms_raw = request.form.get('symptoms') or ''
        selected_symptoms = request.form.getlist('symptoms_list')  # for checkboxes

        # Combine both sources if needed
        text_symptoms = [s.strip() for s in symptoms_raw.split(',') if s.strip()]
        user_symptoms = list(set(text_symptoms + selected_symptoms))

        # Validate symptoms
        invalid_symptoms = [s for s in user_symptoms if s not in symptoms_dict]
        if not user_symptoms or invalid_symptoms:
            message = f"Please enter valid symptom(s). Invalid: {', '.join(invalid_symptoms)}"
        ...
        # Get disease info
        dis_des, precautions_data, medications_data, rec_diet, workout = helper(predicted_disease)
        my_precautions = [i for i in precautions_data[0]]

        return render_template('index.html',
                               predicted_disease=predicted_disease,dis_des=dis_des,my_precautions=my_precautions,
                               medications=medications_data,my_diet=rec_diet,workout=workout,symptoms=symptom_list)
